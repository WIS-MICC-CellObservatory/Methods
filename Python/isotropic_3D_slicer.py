...
This script now offers **three ways to run**:
1) **CLI (default)** — no extra GUI libs required.
2) **PyQt6 GUI** — desktop UI, launch with `--gui`.
3) **Streamlit UI** — web-style UI, if `streamlit` is installed.
...
- Choose compression (lossless) and optionally skip empty slices.
...
def compression_param(name: str):
    mapping = {
        "deflate": "deflate",
        "lzw": "lzw",
        "zstd": "zstd",
    ...
    if name.lower() in ("deflate (zip)", "zip", "deflate"):
        return "deflate"
    key = name.lower()
    if key not in mapping:
        raise ValueError(f"Unknown compression '{name}'. Choose from deflate|lzw|zstd|packbits|none")
    return mapping[key]
...
def save_isotropic_slices(
    ...
    compression = compression_param(compress)
...
def streamlit_main():
    """Run the Streamlit UI if Streamlit is installed."""
    try:
    ...
    st.markdown("Configure parameters, then generate XY/ZX/ZY slices with preserved channel placement.")
    ...
    with col_left:
        ...
        if arr is not None:
            ...
            suggested_c = "none"
            if meta_axes:
                ax = meta_axes.replace("S", "C")
                if "Z" in ax:
                    suggested_z = str(ax.index("Z"))
                if "C" in ax:
                    suggested_c = str(ax.index("C"))
                else:
                    suggested_c = "none"

            z_axis_str = st.text_input("Z axis (index or 'first'/'last')", value=suggested_z)
            c_axis_str = st.text_input("Channel axis (index, 'first'/'last', or 'none')", value=suggested_c)

        st.subheader("Resampling & Output")
        z_aspect = st.number_input("z_aspect = z_spacing / xy_spacing", min_value=0.01, max_value=100.0, value=1.0, step=0.01)
        mode = st.radio("Interpolation mode", options=["labels", "image"], index=0, horizontal=True)
        comp_choice = st.selectbox("Compression", options=["deflate", "lzw", "zstd", "packbits", "none"], index=0)
        out_dir = st.text_input("Output directory", value=str(Path.cwd() / "isotropic_slices"))
        skip_empty = st.checkbox("Skip empty slices (all zeros)", value=False)
    ...
    if run_btn:
        ...
        try:
            ...
            st.success(
                ...
                f"channels={result['cpos']}; mode={result['mode']}; compression={result['compression']}"
            )
        except Exception as e:
            st.error(f"Error: {e}")
            st.exception(e)
...
def qt_main():
    ...
    class MainWindow(QMainWindow):
        def __init__(self):
            ...
            self.in_edit = QLineEdit(); h1.addWidget(self.in_edit)
            ...
            self.z_spin = QDoubleSpinBox(); self.z_spin.setRange(0.01, 100.0); self.z_spin.setSingleStep(0.01); self.z_spin.setValue(1.0); h3.addWidget(self.z_spin)
            ...
            self.out_edit = QLineEdit(str(Path.cwd() / "isotropic_slices")); h4.addWidget(self.out_edit)
            b_out = QPushButton("Choose…"); h4.addWidget(b_out)
            b_out.clicked.connect(self.choose_outdir)

            # --- Options ---
            h5 = QHBoxLayout(); v.addLayout(h5)
            h5.addWidget(QLabel("compression:"))
            self.comp_combo = QComboBox(); self.comp_combo.addItems(["deflate", "lzw", "zstd", "packbits", "none"]) ; h5.addWidget(self.comp_combo)
            self.skip_chk = QCheckBox("skip empty slices"); h5.addWidget(self.skip_chk)
        ...
        def browse_input(self):
            ...
            try:
                ...
                if axes:
                    if "Z" in axes:
                        self.z_combo.setCurrentText(str(axes.index("Z")))
                    if "C" in axes:
                        self.c_combo.setCurrentText(str(axes.index("C")))
            except Exception as e:
        ...
        def run_job(self):
            ...
            params = dict(
                ...
                compress=self.comp_combo.currentText(),
        ...
        def on_finished(self, result: dict):
            ...
            self.log.append(
                ...
                f"channels={result['cpos']}; compression={result['compression']}"
            )
...
def cli_main(argv=None):
    ...
    p.add_argument("--compression", default="deflate", choices=["deflate", "lzw", "zstd", "packbits", "none"], help="TIFF compression")
